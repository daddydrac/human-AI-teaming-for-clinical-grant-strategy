...
pub fn write_retrieval_parquet(
    path: &Path,
    records: &[crate::domain::RetrievalRecord],
) -> Result<()> {
    ...
    let req = StringArray::from(
        records
            .iter()
            .map(|r| r.requirement_id.as_deref())
            .collect::<Vec<_>>(),
    );
    let src = StringArray::from(
        records
            .iter()
            .map(|r| r.source_ref.as_str())
            .collect::<Vec<_>>(),
    );
    let url = StringArray::from(
        records
            .iter()
            .map(|r| r.source_url.as_deref())
            .collect::<Vec<_>>(),
    );
    let loc = StringArray::from(
        records
            .iter()
            .map(|r| r.source_locator.as_deref())
            .collect::<Vec<_>>(),
    );
