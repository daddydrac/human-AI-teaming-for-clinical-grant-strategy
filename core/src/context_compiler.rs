...
pub async fn compile(
    ...
    let k = std::env::var("CONTEXT_RETRIEVAL_K")
        .ok()
    ...
    out.push_str(&req);
    let workflow=store.workflow_config(project)?;
    if workflow.enabled("clinical_design"){
    ...
    if workflow.enabled("sponsor_compliance")&&remaining_for_compliance > 2_000 {
...
",
    ...
    if workflow.enabled("competitive_intelligence")&&remaining_for_competitive > 2_000 {
    ...
    for hit in &hits {
        ...
        if out.len() + block.len() > token_budget_chars {
            break;
        }
        out.push_str(&block);
    ...
    Ok(CompiledContext {
        text: out,
        retrieved: serde_json::json!(rows),
    })
}
