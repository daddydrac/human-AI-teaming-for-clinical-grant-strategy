...
use crate::{
    ...
    models::{GenerationAudit, ModelRouter, ModelTask},
...
impl CompetitiveEngine {
    pub fn from_env(
        ...
        let mut config: CompetitiveConfig =
            serde_json::from_str(&raw).context("parse competitive intelligence config")?;
        validate_config(&mut config)?;
        let timeout = std::env::var("COMPETITIVE_HTTP_TIMEOUT_SECONDS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(45u64);
        let user_agent = std::env::var("COMPETITIVE_USER_AGENT")
            .unwrap_or_else(|_| format!("GrantWriterCompetitive/{}", env!("CARGO_PKG_VERSION")));
        let client = Client::builder()
            .timeout(Duration::from_secs(timeout))
            .user_agent(user_agent)
            .build()?;
        let mut aliases = std::env::var("COMPETITIVE_HOME_ORGANIZATION_ALIASES")
            .ok()
            .unwrap_or_else(|| std::env::var("ORGANIZATION_NAME").unwrap_or_default())
            .split(',')
            .map(str::trim)
            .filter(|x| !x.is_empty())
            .map(ToOwned::to_owned)
            .collect::<Vec<_>>();
        if let Ok(home) = std::env::var("ORGANIZATION_NAME") {
        ...
        Ok(Self {
            ...
            openalex_mailto: std::env::var("OPENALEX_MAILTO")
                .ok()
                .filter(|x| !x.trim().is_empty()),
            openalex_api_key: std::env::var("OPENALEX_API_KEY")
                .ok()
                .filter(|x| !x.trim().is_empty()),
        })
    }
    pub fn config_sha256(&self) -> Result<String> {
        Ok(sha256_hex(&serde_json::to_vec(&self.config)?))
    }
...
{}"#,
        ...
        validate_profile(
            &mut env.profile,
            self.config.limits.max_profile_search_queries,
        )?;
    ...
    pub async fn run(
        ...
        own_context: &str,
    ) -> Result<CompetitiveRunOutput> {
        let mut provider_status = Vec::new();
        let mut assets = Vec::new();
        let specs = profile
            .search_queries
            .iter()
            .take(self.config.limits.max_profile_search_queries)
            .cloned()
            .collect::<Vec<_>>();
        if self.config.providers.nih_reporter {
            ...
            for spec in specs
                .iter()
                .filter(|s| s.source_types.iter().any(|x| x == "nih_grants"))
            {
                match self.nih_reporter(spec).await {
                    Ok(mut x) => assets.append(&mut x),
                    Err(e) => errors.push(e.to_string()),
                }
                sleep(Duration::from_millis(
                    self.config.rate_limits.nih_reporter_min_interval_ms,
                ))
                .await;
            }
        ...
        if self.config.providers.clinical_trials {
            ...
            for spec in specs
                .iter()
                .filter(|s| s.source_types.iter().any(|x| x == "clinical_trials"))
            {
                match self.clinical_trials(spec).await {
                    Ok(mut x) => assets.append(&mut x),
                    Err(e) => errors.push(e.to_string()),
                }
                sleep(Duration::from_millis(
                    self.config.rate_limits.clinical_trials_min_interval_ms,
                ))
                .await;
            }
        ...
        if self.config.providers.openalex {
            ...
            if self.openalex_api_key.is_none() {
                errors.push(
                    "OPENALEX_API_KEY is not configured; publication discovery was skipped".into(),
                );
            } else {
                for spec in specs
                    .iter()
                    .filter(|s| s.source_types.iter().any(|x| x == "publications"))
                {
                    match self.openalex(spec).await {
                        Ok(mut x) => assets.append(&mut x),
                        Err(e) => errors.push(e.to_string()),
                    }
                    sleep(Duration::from_millis(
                        self.config.rate_limits.openalex_min_interval_ms,
                    ))
                    .await;
                }
            }
        ...
        cap_assets(
            &mut assets,
            self.config.limits.max_assets_per_candidate,
            self.config.limits.max_assets_total,
        );

        let top_for_enrichment =
            preselect_candidates(&assets, self.config.limits.enrich_top_candidates);
        let enrichment_candidates = top_for_enrichment
            .iter()
            .filter_map(|key| {
                assets
                    .iter()
                    .find(|a| &a.candidate_key == key)
                    .map(|a| (key.clone(), a.candidate_name.clone()))
            })
            .collect::<Vec<_>>();
        if self.config.providers.ip_web {
            let before = assets.len();
            let (mut enriched, errors) = self
                .web_enrich_many(profile, &enrichment_candidates, true)
                .await;
        ...
        if self.config.providers.technology_web {
            let before = assets.len();
            let (mut enriched, errors) = self
                .web_enrich_many(profile, &enrichment_candidates, false)
                .await;
            assets.append(&mut enriched);
        ...
        if candidates.is_empty() {
            bail!("public competitive-intelligence providers returned no capability-matched candidate organizations above the configured relevance threshold");
        }
        let (strategy, strategy_model) = self
            .synthesize_strategy(audit, project_id, profile, &candidates, &assets, own_context)
            .await?;
        Ok(CompetitiveRunOutput {
            candidates,
            assets,
            provider_status,
            strategy,
    ...
    async fn nih_reporter(&self, spec: &CompetitiveSearchSpec) -> Result<Vec<PublicAsset>> {
        let limit = self
            .config
            .limits
            .nih_reporter_results_per_query
            .clamp(1, 500);
        let payload = json!({
        ...
        let v: Value = self
            .client
            .post(&self.config.endpoints.nih_reporter_projects)
            .json(&payload)
            .send()
            .await?
            .error_for_status()?
        ...
        for r in rows {
            let org = r
                ...
                .or_else(|| r.pointer("/organization/name").and_then(Value::as_str))
                .unwrap_or("")
                .trim()
                .to_string();
            if org.is_empty() {
            ...
            let external = r
                ...
                .unwrap_or_else(|| sha256_hex(title.as_bytes()));
            let url = r
            ...
            let year = r
                ...
                .map(|x| x as i32);
            let amount = r.get("total_cost").and_then(Value::as_f64).or_else(|| {
                r.get("total_cost")
                    .and_then(Value::as_i64)
                    .map(|x| x as f64)
            });
            let key = self.organization_key(&org);
            if key.is_empty() {
                continue;
            }
            out.push(make_asset(
    ...
    async fn clinical_trials(&self, spec: &CompetitiveSearchSpec) -> Result<Vec<PublicAsset>> {
        let page_size = self
            .config
            .limits
            .clinical_trials_results_per_query
            .clamp(1, 1000);
        let params = vec![
            ("query.term".to_string(), spec.query.clone()),
            ("pageSize".to_string(), page_size.to_string()),
            ("format".to_string(), "json".to_string()),
        ];
        let v: Value = self
            .client
            .get(&self.config.endpoints.clinical_trials_studies)
            .query(&params)
            .send()
            .await?
            .error_for_status()?
        ...
        for study in rows {
            ...
            let title = study
                ...
                .unwrap_or("Clinical study");
            let brief = study
                .pointer("/protocolSection/descriptionModule/briefSummary")
                .and_then(Value::as_str)
                .unwrap_or("");
            let detail = study
                .pointer("/protocolSection/descriptionModule/detailedDescription")
                .and_then(Value::as_str)
                .unwrap_or("");
            ...
            if let Some(x) = study
                ...
                .and_then(Value::as_str)
            {
                orgs.push(x.to_string());
            }
            if let Some(a) = study
                .pointer("/protocolSection/sponsorCollaboratorsModule/collaborators")
                .and_then(Value::as_array)
            {
                for x in a {
                    if let Some(n) = x.get("name").and_then(Value::as_str) {
    ...
    async fn openalex(&self, spec: &CompetitiveSearchSpec) -> Result<Vec<PublicAsset>> {
        ...
        let mut params = vec![
            ("search".to_string(), spec.query.clone()),
            ("per-page".to_string(), per_page.to_string()),
            (
                "select".to_string(),
                "id,doi,title,publication_year,cited_by_count,authorships,primary_topic,type"
                    .to_string(),
            ),
        ];
        if let Some(mail) = self.openalex_mailto.as_deref() {
            params.push(("mailto".to_string(), mail.to_string()));
        }
        ...
        let req = self
            .client
            .get(&self.config.endpoints.openalex_works)
            .query(&params);
        ...
        let mut out = Vec::new();
        for work in rows {
            let id = work.get("id").and_then(Value::as_str).unwrap_or("");
            if id.is_empty() {
                continue;
            }
            let title = work
            ...
            if let Some(authorships) = work.get("authorships").and_then(Value::as_array) {
                for auth in authorships {
                    if let Some(insts) = auth.get("institutions").and_then(Value::as_array) {
                        for inst in insts {
                            if let Some(name) = inst.get("display_name").and_then(Value::as_str) {
                                let iid = inst.get("id").and_then(Value::as_str).unwrap_or(name);
                                institutions
                                    .entry(iid.to_string())
            ...
            for (_iid, org) in institutions {
                ...
                out.push(make_asset(
                    ...
                    title,
                    &summary,
                    doi.clone().or_else(|| Some(id.to_string())),
                    year,
                    None,
                    Some(spec.dimension_id.clone()),
                    work.clone(),
    ...
    async fn web_enrich_many(
        &self,
        profile: &CompetitiveProfile,
        candidates: &[(String, String)],
        ip: bool,
    ) -> (Vec<PublicAsset>, Vec<String>) {
    ...
    async fn web_enrich(
        ...
        candidate_name: &str,
        ip: bool,
    ) -> Result<Vec<PublicAsset>> {
        let terms = if ip {
            &profile.ip_terms
        } else {
            &profile.technology_terms
        };
        let term_text = terms
            .iter()
            .take(6)
        ...
        let query = if ip {
            format!(
                "\"{}\" (patent OR patents OR intellectual property OR licensing) {}",
                candidate_name, term_text
            )
        } else {
            format!(
                "\"{}\" (technology OR platform OR translational OR licensing OR partnership) {}",
                candidate_name, term_text
            )
        };
        let domains = if ip {
            &self.config.ip_search_domains
        } else {
            &self.config.technology_search_domains
        };
        let count = if ip {
            self.config.limits.ip_web_results_per_candidate
        } else {
            self.config.limits.technology_web_results_per_candidate
        };
    ...
    async fn score_assets(
        &self,
        profile: &CompetitiveProfile,
        assets: &mut [PublicAsset],
    ) -> Result<()> {
        if assets.is_empty() {
            return Ok(());
        }
        ...
        let dims = vectors
            .first()
            .context("embedding endpoint returned zero public asset vectors")?
            .len();
        if q.len() != dims {
            bail!(
                "competitive embedding dimension mismatch: query {} vs assets {}",
                q.len(),
                dims
            );
        }
        if vectors.iter().any(|v| v.len() != dims) {
            bail!("competitive asset embedding dimensions are inconsistent");
        }
    ...
    async fn synthesize_strategy(
        ...
        assets: &[PublicAsset],
        own_context: &str,
    ) -> Result<(CompetitiveStrategy, String)> {
        let selected = candidates
            .iter()
            .take(self.config.limits.strategy_candidates)
            .cloned()
            .collect::<Vec<_>>();
        ...
        for c in &selected {
            let mut a = assets
                .iter()
                .filter(|x| x.candidate_key == c.candidate_key)
                .cloned()
                .collect::<Vec<_>>();
            a.sort_by(|x, y| y.relevance.total_cmp(&x.relevance));
            a.truncate(self.config.limits.strategy_assets_per_candidate);
            chosen_assets.extend(a);
        }
        ...
        if packet_text.len() > self.config.limits.strategy_max_chars {
            packet_text.truncate(self.config.limits.strategy_max_chars);
        }
        let prompt = format!(
            r#"Develop an evidence-bounded grant positioning strategy from public competitive intelligence. The listed organizations are POTENTIAL capability-matched competitors, not confirmed applicants. Never state or imply that they will apply. Never invent confidential information, unpublished IP, private technology, unpublished trial data, or applicant intent.
...
{}"#,
    ...
    fn organization_key(&self, name: &str) -> String {
        normalize_organization(name, &self.config.organization_suffixes)
    }
    fn is_home_organization(&self, name: &str) -> bool {
        let key = self.organization_key(name);
        if key.len() < 5 {
            return false;
        }
        self.home_aliases.iter().any(|a| {
            let x = self.organization_key(a);
            x.len() >= 5 && (key == x || key.contains(&x) || x.contains(&key))
        })
    }
}
...
fn validate_config(c: &mut CompetitiveConfig) -> Result<()> {
    if c.schema_version == 0 {
        bail!("competitive intelligence config schema_version must be positive");
    }
    if c.limits.max_candidates == 0
        || c.limits.max_assets_total == 0
        || c.limits.max_assets_per_candidate == 0
        || c.limits.web_enrichment_concurrency == 0
    {
        bail!("competitive intelligence limits must be positive");
    }
    if c.scoring.top_assets_per_type == 0 || c.scoring.count_saturation <= 0.0 {
        bail!("competitive scoring configuration is invalid");
    }
    if c.scoring.asset_type_weights.values().any(|x| *x < 0.0) {
        bail!("competitive asset weights must be non-negative");
    }
    if c.scoring
        .provider_reliability
        .values()
        .any(|x| *x < 0.0 || *x > 1.0)
    {
        bail!("competitive provider reliability values must be between 0 and 1");
    }
    if c.updates.max_sections_per_refresh == 0 {
        bail!("competitive updates max_sections_per_refresh must be positive");
    }
    if !c.updates.candidate_score_delta.is_finite()
        || c.updates.candidate_score_delta < 0.0
        || c.updates.candidate_score_delta > 1.0
    {
        bail!("competitive updates candidate_score_delta must be between 0 and 1");
    }
    for u in [
        &c.endpoints.nih_reporter_projects,
        &c.endpoints.clinical_trials_studies,
        &c.endpoints.openalex_works,
    ] {
        let parsed = reqwest::Url::parse(u)?;
        if parsed.scheme() != "https" {
            bail!("competitive public API endpoint must use https: {u}");
        }
    }
    Ok(())
}
...
fn validate_profile(p: &mut CompetitiveProfile, max_queries: usize) -> Result<()> {
    if p.summary.trim().is_empty() {
        bail!("competitive profile summary is empty");
    }
    if p.capability_dimensions.is_empty() {
        bail!("competitive profile contains no capability dimensions");
    }
    let mut ids = BTreeSet::new();
    let mut sum = 0.0f32;
    for d in &mut p.capability_dimensions {
        d.id = d.id.trim().to_string();
        d.label = d.label.trim().to_string();
        d.description = d.description.trim().to_string();
        if d.id.is_empty() || d.label.is_empty() || d.description.is_empty() || d.weight <= 0.0 {
            bail!("competitive capability dimensions require id, label, description and positive weight");
        }
        if !ids.insert(d.id.clone()) {
            bail!("duplicate competitive capability dimension id {}", d.id);
        }
        sum += d.weight;
    }
    for d in &mut p.capability_dimensions {
        d.weight /= sum.max(f32::EPSILON);
    }
    let allowed = BTreeSet::from(["nih_grants", "publications", "clinical_trials"]);
    p.search_queries
        .retain(|q| !q.query.trim().is_empty() && ids.contains(&q.dimension_id));
    for q in &mut p.search_queries {
        q.source_types.retain(|x| allowed.contains(x.as_str()));
        q.source_types.sort();
        q.source_types.dedup();
    }
    p.search_queries.retain(|q| !q.source_types.is_empty());
    p.search_queries.truncate(max_queries);
    if p.search_queries.is_empty() {
        bail!("competitive profile contains no valid public search queries");
    }
    Ok(())
}

fn validate_strategy(
    s: &mut CompetitiveStrategy,
    candidates: &BTreeSet<&str>,
    assets: &[PublicAsset],
) -> Result<()> {
    let asset_keys = assets
        .iter()
        .map(|x| x.asset_key.as_str())
        .collect::<BTreeSet<_>>();
    let valid_conf = BTreeSet::from(["high", "medium", "low"]);
    for d in &mut s.differentiators {
        d.candidate_keys.retain(|x| candidates.contains(x.as_str()));
        d.asset_keys.retain(|x| asset_keys.contains(x.as_str()));
        d.candidate_keys.sort();
        d.candidate_keys.dedup();
        d.asset_keys.sort();
        d.asset_keys.dedup();
        if !valid_conf.contains(d.confidence.as_str()) {
            d.confidence = "low".into();
        }
    }
    s.differentiators
        .retain(|d| d.candidate_keys.is_empty() || !d.asset_keys.is_empty());
    for g in &mut s.gaps_to_close {
        g.candidate_keys.retain(|x| candidates.contains(x.as_str()));
        g.asset_keys.retain(|x| asset_keys.contains(x.as_str()));
        g.candidate_keys.sort();
        g.candidate_keys.dedup();
        g.asset_keys.sort();
        g.asset_keys.dedup();
    }
    s.gaps_to_close
        .retain(|g| g.candidate_keys.is_empty() || !g.asset_keys.is_empty());
    s.candidate_notes
        .retain(|n| candidates.contains(n.candidate_key.as_str()));
    for n in &mut s.candidate_notes {
        n.asset_keys.retain(|x| asset_keys.contains(x.as_str()));
        n.asset_keys.sort();
        n.asset_keys.dedup();
    }
    s.candidate_notes.retain(|n| !n.asset_keys.is_empty());
    for g in &mut s.section_guidance {
        g.asset_keys.retain(|x| asset_keys.contains(x.as_str()));
        g.asset_keys.sort();
        g.asset_keys.dedup();
    }
...
fn make_asset(
    ...
    let asset_key =
        sha256_hex(format!("{provider}|{asset_type}|{external_id}|{candidate_key}").as_bytes());
    PublicAsset {
        asset_key,
        candidate_key: candidate_key.to_string(),
        candidate_name: candidate_name.trim().to_string(),
...
fn status(provider: &str, records: usize, errors: Vec<String>) -> ProviderStatus {
    ProviderStatus {
        provider: provider.into(),
        ok: errors.is_empty(),
        records,
...
pub fn normalize_organization(name: &str, suffixes: &[String]) -> String {
    ...
    let mut tokens = cleaned
        ...
        .map(ToOwned::to_owned)
        .collect::<Vec<_>>();
    while tokens.last().is_some_and(|x| suffix.contains(x)) {
        tokens.pop();
    }
    tokens.join(" ")
}
fn dedup_assets(assets: &mut Vec<PublicAsset>) {
    let mut seen = BTreeSet::new();
    assets.retain(|a| seen.insert((a.asset_key.clone(), a.candidate_key.clone())));
}
fn preselect_candidates(assets: &[PublicAsset], limit: usize) -> BTreeSet<String> {
    ...
    for a in assets {
        let e = by.entry(a.candidate_key.clone()).or_default();
        e.count += 1;
        e.types.insert(a.asset_type.clone());
        if let Some(d) = a.dimension_id.as_ref() {
            e.dimensions.insert(d.clone());
        }
    }
    ...
    x.sort_by(|a, b| {
        b.1.dimensions
            .len()
            .cmp(&a.1.dimensions.len())
            .then_with(|| b.1.types.len().cmp(&a.1.types.len()))
...
fn cap_assets(assets: &mut Vec<PublicAsset>, per_candidate: usize, total: usize) {
    ...
    for a in input {
        by_candidate
            .entry(a.candidate_key.clone())
            .or_default()
            .entry(a.asset_type.clone())
            .or_default()
            .push_back(a);
    }
    ...
    for (candidate, mut types) in by_candidate {
        ...
        while keep.len() < per_candidate {
            let mut added = false;
            for queue in types.values_mut() {
                if keep.len() >= per_candidate {
                    break;
                }
    ...
    while assets.len() < total {
        let mut added = false;
        for queue in balanced.values_mut() {
            if assets.len() >= total {
                break;
            }
            if let Some(a) = queue.pop_front() {
...
fn profile_search_text(p: &CompetitiveProfile) -> String {
...
fn asset_embedding_text(a: &PublicAsset) -> String {
    format!(
        "Organization: {}\nAsset type: {}\nTitle: {}\nSummary: {}",
        a.candidate_name, a.asset_type, a.title, a.summary
    )
}
...
fn aggregate_candidates(
    ...
    let mut by = HashMap::<String, Vec<&PublicAsset>>::new();
    for a in assets {
        by.entry(a.candidate_key.clone()).or_default().push(a);
    }
    let mut out = Vec::new();
    for (key, aa) in by {
        let name = aa
            .first()
            .map(|x| x.candidate_name.clone())
            .unwrap_or_else(|| key.clone());
        let mut type_scores = BTreeMap::<String, f32>::new();
        let mut counts = BTreeMap::<String, usize>::new();
        for typ in cfg.asset_type_weights.keys() {
            let mut vals = aa
                .iter()
                .filter(|a| &a.asset_type == typ)
                .map(|a| a.relevance)
                .collect::<Vec<_>>();
            vals.sort_by(|a, b| b.total_cmp(a));
            let count = vals.len();
            counts.insert(typ.clone(), count);
            let top = vals
                .iter()
                .take(cfg.top_assets_per_type)
                .copied()
                .collect::<Vec<_>>();
            let mean = if top.is_empty() {
                0.0
            } else {
                top.iter().sum::<f32>() / (top.len() as f32)
            };
            let sat = 1.0 - (-(count as f32) / cfg.count_saturation).exp();
            type_scores.insert(typ.clone(), (mean * (0.75 + 0.25 * sat)).clamp(0.0, 1.0));
        }
        let weight_sum = cfg
            .asset_type_weights
            .values()
        ...
        let base = cfg
            .asset_type_weights
            .iter()
            .map(|(t, w)| w * type_scores.get(t).copied().unwrap_or(0.0))
            .sum::<f32>()
            / weight_sum;
        let active = cfg
            .asset_type_weights
            ...
            .count();
        let breadth = (active as f32) / (cfg.asset_type_weights.len().max(1) as f32);
        let bw = cfg.breadth_weight.clamp(0.0, 0.5);
        let overall = ((1.0 - bw) * base + bw * breadth).clamp(0.0, 1.0);
        let dimensions = profile
            ...
            .map(|d| {
                let mut x = aa
                    .iter()
                    .filter(|a| a.dimension_id.as_deref() == Some(d.id.as_str()))
                    .collect::<Vec<_>>();
                x.sort_by(|a, b| b.relevance.total_cmp(&a.relevance));
                DimensionCoverage {
                    dimension_id: d.id.clone(),
                    label: d.label.clone(),
                    score: x.first().map(|a| a.relevance).unwrap_or(0.0),
                    supporting_asset_keys: x
                        .into_iter()
                        .take(3)
        ...
        out.push(CandidateScore {
            ...
            grant_score: type_scores.get("grant").copied().unwrap_or(0.0),
            publication_score: type_scores.get("publication").copied().unwrap_or(0.0),
            clinical_trial_score: type_scores.get("clinical_trial").copied().unwrap_or(0.0),
            patent_ip_score: type_scores.get("patent_ip").copied().unwrap_or(0.0),
            technology_score: type_scores.get("technology").copied().unwrap_or(0.0),
...
mod tests {
    use super::*;
    #[test]
    fn organization_normalization() {
        let s = vec!["INC".into(), "LLC".into()];
        assert_eq!(normalize_organization("Acme Bio, Inc.", &s), "ACME BIO");
        assert_eq!(
            normalize_organization("University of Example", &s),
            "UNIVERSITY OF EXAMPLE"
        );
    }
    #[test]
    fn profile_validation_normalizes_weights() {
        let mut p = CompetitiveProfile {
            summary: "x".into(),
            likely_applicant_types: vec![],
            ...
            disease_terms: vec![],
            technology_terms: vec![],
            clinical_terms: vec![],
            ip_terms: vec![],
            grant_terms: vec![],
            search_queries: vec![CompetitiveSearchSpec {
                dimension_id: "A".into(),
                query: "query".into(),
                source_types: vec!["nih_grants".into()],
            }],
        };
        validate_profile(&mut p, 12).unwrap();
        assert!(
    ...
    #[test]
    fn asset_cap_preserves_type_breadth() {
        let mut a = Vec::new();
        for i in 0..10 {
            a.push(make_asset(
                "ORG",
        ...
        let types = a
            .iter()
            .map(|x| x.asset_type.as_str())
            .collect::<BTreeSet<_>>();
        assert!(
            types.contains("grant") && types.contains("patent_ip") && types.contains("technology")
        );
    }
}
