...
impl ResearchClient {
    ...
    pub async fn ingestion_health(&self)->Result<serde_json::Value>{
        let mut url=Url::parse(&self.ingestion_url).context("invalid DOCUMENT_INGESTION_URL")?;url.set_path("/health");url.set_query(None);
    ...
    pub fn from_env() -> Result<Self> {
        ...
        let user_agent=std::env::var("RESEARCH_USER_AGENT").unwrap_or_else(|_|format!("GrantWriter/{}",env!("CARGO_PKG_VERSION")));
        let http = Client::builder()
            .timeout(Duration::from_secs(timeout))
            .user_agent(&user_agent)
            .redirect(reqwest::redirect::Policy::none())
            .build()?;
        let ingestion_timeout=std::env::var("INGESTION_HTTP_TIMEOUT_SECONDS").ok().and_then(|v|v.parse().ok()).unwrap_or(90u64).clamp(10,300);
        let ingestion_http=Client::builder().timeout(Duration::from_secs(ingestion_timeout)).user_agent(user_agent).redirect(reqwest::redirect::Policy::none()).build()?;
        Ok(Self {
            http,
            ingestion_http,
            ...
            ingestion_url: std::env::var("DOCUMENT_INGESTION_URL").unwrap_or_else(|_| "http://ingestion:8091/extract".into()),
            max_concurrency,
    ...
    pub async fn fetch_rendered(&self, url:&str, title_hint:Option<&str>)->Result<FetchedSource>{
        let parsed=Url::parse(url).context("invalid source URL")?;
        if !matches!(parsed.scheme(),"http"|"https"){bail!("unsupported source URL scheme");}
        self.validate_public_destination(&parsed).await?;
        let response=self.ingestion_http.post(&self.ingestion_url).json(&serde_json::json!({"url":url})).send().await?;
        let status=response.status();
        if !status.is_success(){let detail=response.text().await.unwrap_or_default();bail!("browser ingestion returned HTTP {status}: {detail}");}
        #[derive(Deserialize)]
        struct Rendered {title:String,url:String,text:String,status:u16}
        let rendered:Rendered=response.json().await.context("browser ingestion returned invalid JSON")?;
        if rendered.text.trim().is_empty(){bail!("browser ingestion returned no readable Markdown");}
        if rendered.text.as_bytes().len()>self.max_body_bytes{bail!("rendered Markdown exceeds configured limit of {} bytes",self.max_body_bytes);}
        let mut h=Sha256::new();h.update(rendered.text.as_bytes());
        Ok(FetchedSource{title:title_hint.filter(|x|!x.trim().is_empty()).unwrap_or(&rendered.title).to_owned(),url:rendered.url,text:rendered.text,retrieved_at:OffsetDateTime::now_utc().format(&time::format_description::well_known::Rfc3339)?,sha256:hex::encode(h.finalize()),status:rendered.status})
    }
}
