...
use crate::compliance::{
    ComplianceDraftEnvelope, ComplianceProfile, ComplianceProfileDraft, ComplianceRule,
    ComplianceRuleDraft, SOURCE_NOT_LOCATED,
};
...
fn normalized_projection(input: &str) -> Projection {
    ...
    while i < chars.len() {
        ...
        if ch == '-' {
            ...
            if newline && j < chars.len() && chars[j].1.is_alphanumeric() {
                i = j;
                continue;
            }
        }
...
fn terms(input: &str) -> HashSet<String> {
    ...
    normalized_projection(input)
        .text
        .split_whitespace()
        .map(canonical_term)
        .filter(|x| x.len() > 1 && !STOP.contains(&x.as_str()))
        .collect()
}
...
fn candidate_spans(text: &str) -> Vec<(usize, usize)> {
    ...
    for (idx, ch) in text.char_indices() {
        ...
        if matches!(ch, '.' | '?' | '!' | ';' | '\n' | '\u{000c}') {
            if let Some(span) = trim_span(text, sentence_start, end) {
                if span.1 - span.0 >= 12 && span.1 - span.0 <= 1600 {
                    spans.insert(span);
                }
            }
            sentence_start = end;
        ...
        if ch == '\u{000c}' || (ch == '\n' && previous_newline) {
            if let Some(span) = trim_span(text, paragraph_start, idx) {
                if span.1 - span.0 >= 12 && span.1 - span.0 <= 2400 {
                    spans.insert(span);
                }
            }
            paragraph_start = end;
    ...
    for line in text.split_inclusive('\n') {
        ...
        if let Some(span) = trim_span(text, offset, end) {
            if span.1 - span.0 >= 12 && span.1 - span.0 <= 1600 {
                spans.insert(span);
            }
        }
...
fn derived_page(text: &str, start: usize) -> Option<u32> {
    text.contains('\u{000c}')
        .then(|| text[..start].chars().filter(|c| *c == '\u{000c}').count() as u32 + 1)
}
...
fn locate_rule(rule: &ComplianceRuleDraft, documents: &[PreparedDocument<'_>]) -> Option<Match> {
    ...
    let document_hint = rule
        .source_document_hint
        .as_deref()
        .map(|x| normalized_projection(x).text)
        .filter(|x| !x.is_empty());
    let mut ranked = Vec::<Match>::new();
    for prepared in documents {
        let doc = prepared.source;
        let hint_anchor = (!hint_norm.is_empty())
            .then(|| prepared.projection.text.find(&hint_norm))
            .flatten()
            .and_then(|start| {
                prepared
                    .projection
                    .original_range(start, start + hint_norm.len())
            });
        for candidate in &prepared.candidates {
            ...
            let exact_hint = hint_norm.split_whitespace().count() >= 3
                && candidate.normalized.contains(&hint_norm);
            if hint_found == 0 || (!target_terms.is_empty() && target_found == 0) {
                continue;
            }
            if let Some(n) = numeric.as_deref() {
                if !candidate.terms.contains(&canonical_term(n)) {
                    continue;
                }
            }
            let page = derived_page(&doc.text, candidate.start);
            if let Some(wanted) = rule.source_page_hint {
                if page.is_some() && page != Some(wanted) {
                    continue;
                }
            }
            let mut score = ratio(hint_found, hint_terms.len()) * 0.48
                + ratio(target_found, target_terms.len()) * 0.24;
            ...
            if hint_anchor
                .is_some_and(|(start, end)| candidate.start <= start && candidate.end >= end)
            {
                score += 0.12;
            }
            if document_hint
                .as_ref()
                .is_some_and(|h| prepared.name_normalized.contains(h))
            {
                score += 0.10;
            }
            // Prefer a concise supporting span when two candidates cover the same terms.
            score -= (candidate.end - candidate.start)
                .saturating_sub(500)
                .min(1900) as f64
                / 1900.0
                * 0.08;
    ...
    if ranked.iter().skip(1).any(|other| {
        let overlaps = other.document_id == best.document_id
            && other.start < best.end
            && best.start < other.end;
    ...
    Some(Match {
        ...
        document_name: best.document_name.clone(),
        start: best.start,
        end: best.end,
        page: best.page,
        score: best.score,
...
/// Locate a model-extracted statement in immutable source buffers. The model
/// supplies search wording only; this function copies the exact source bytes
/// and fails closed when similarly-scored passages make the match ambiguous.
pub fn locate_statement(
    statement: &str,
    source_clue: Option<&str>,
    documents: &[SourceDocument],
) -> Option<LocatedExcerpt> {
    let prepared = documents
        .iter()
        .map(|source| PreparedDocument {
            source,
            name_normalized: normalized_projection(&format!(
                "{} {} {}",
        ...
        .collect::<Vec<_>>();
    let rule = ComplianceRuleDraft {
        rule_id: "source-location".into(),
        category: "source".into(),
        rule_type: "required_section".into(),
        scope: "solicitation".into(),
        target: statement.into(),
        severity: "hard".into(),
        mandatory: true,
        numeric_value: None,
        text_value: None,
        list_value: Vec::new(),
        source_hint: source_clue.unwrap_or(statement).into(),
        source_document_hint: None,
        source_page_hint: None,
        notes: String::new(),
    };
    let found = locate_rule(&rule, &prepared)?;
    let document = documents.iter().find(|item| item.id == found.document_id)?;
    let excerpt = document.text.get(found.start..found.end)?.to_owned();
    Some(LocatedExcerpt {
...
fn not_located(rule: ComplianceRuleDraft) -> ComplianceRule {
    ComplianceRule {
        ...
        list_value: rule.list_value,
        source_hint: rule.source_hint,
        source_document_hint: rule.source_document_hint,
        source_page_hint: rule.source_page_hint,
        source_excerpt: SOURCE_NOT_LOCATED.into(),
        source_locator: SOURCE_NOT_LOCATED.into(),
        source_start_offset: None,
        source_end_offset: None,
        source_document_id: None,
        source_page: None,
        source_status: "not_located".into(),
        notes: rule.notes,
    }
}
...
pub fn locate_profile(
    ...
    let rules = draft
        ...
        .into_par_iter()
        .map(|rule| {
            let Some(found) = locate_rule(&rule, &prepared) else {
                return not_located(rule);
            };
            let Some(doc) = documents.iter().find(|d| d.id == found.document_id) else {
                return not_located(rule);
            };
            let Some(excerpt) = doc.text.get(found.start..found.end) else {
                return not_located(rule);
            };
            if excerpt.is_empty() || !doc.text.contains(excerpt) {
                return not_located(rule);
            }
            ComplianceRule {
                rule_id: rule.rule_id,
                ...
                source_start_offset: Some(found.start),
                source_end_offset: Some(found.end),
                source_document_id: Some(found.document_id),
    ...
    ComplianceProfile {
        ...
        submission_system: draft.submission_system,
        deadline_iso: draft.deadline_iso,
        rules,
    }
}
...
pub fn validate_exact_sources(
    profile: &ComplianceProfile,
    documents: &[SourceDocument],
) -> Result<()> {
    for rule in &profile.rules {
        if rule.source_status == "not_located" {
            continue;
        }
        let document_id = rule
            .source_document_id
            .ok_or_else(|| anyhow::anyhow!("rule {} has no source document", rule.rule_id))?;
        let doc = documents
            .iter()
            .find(|d| d.id == document_id)
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "rule {} references unknown funding-opportunity document {}",
                    rule.rule_id,
                    document_id
                )
            })?;
        let start = rule
            .source_start_offset
            .ok_or_else(|| anyhow::anyhow!("rule {} has no source start offset", rule.rule_id))?;
        let end = rule
            .source_end_offset
            .ok_or_else(|| anyhow::anyhow!("rule {} has no source end offset", rule.rule_id))?;
        let exact = doc.text.get(start..end).ok_or_else(|| {
            anyhow::anyhow!(
                "rule {} source offsets are not valid UTF-8 boundaries",
                rule.rule_id
            )
        })?;
        if exact != rule.source_excerpt || !doc.text.contains(&rule.source_excerpt) {
            bail!("rule {} source_excerpt is not the exact source characters at its persisted offsets",rule.rule_id);
        }
...
mod tests {
    ...
    fn draft(hint: &str) -> ComplianceProfileDraft {
        ComplianceProfileDraft {
            sponsor: None,
            mechanism: None,
            submission_system: None,
            deadline_iso: None,
            rules: vec![ComplianceRuleDraft {
                rule_id: "C-001".into(),
                category: "format".into(),
                rule_type: "max_pages".into(),
                scope: "section".into(),
                target: "Research Strategy".into(),
                severity: "hard".into(),
                mandatory: true,
                ...
                source_document_hint: Some("notice.html".into()),
                source_page_hint: None,
                notes: String::new(),
            }],
        }
    ...
    fn copies_exact_characters_from_url_extracted_buffer() {
        ...
        let docs = vec![SourceDocument {
            id: 42,
            name: "notice.html".into(),
            kind: "funding_url".into(),
            text: text.into(),
            sha256: sha256(text),
        }];
        ...
        assert_eq!(
            &text[rule.source_start_offset.unwrap()..rule.source_end_offset.unwrap()],
            rule.source_excerpt
        );
        assert_eq!(rule.source_document_id, Some(42));
        assert_eq!(rule.source_page, None);
    ...
    fn fails_closed_when_source_cannot_be_located() {
        let docs = vec![SourceDocument {
            ...
            name: "notice.html".into(),
            kind: "funding_url".into(),
            text: "No page limit appears here.".into(),
            sha256: sha256("No page limit appears here."),
        }];
        let profile = locate_profile(draft("Research Strategy page limitation"), &docs);
