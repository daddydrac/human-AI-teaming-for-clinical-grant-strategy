    ...
    v.get("provider_status")
        ...
        .filter_map(|p| {
            Some((
                ...
                p.get("ok").and_then(Value::as_bool).unwrap_or(false),
            ))
        })
        .collect()
}
...
fn strategy_section_guidance(v: &Value) -> BTreeMap<String, String> {
    ...
    for x in v
        .get("strategy")
        .and_then(|s| s.get("section_guidance"))
        .and_then(Value::as_array)
        .into_iter()
        .flatten()
    {
...
pub fn diff(previous: &Value, current: &Value, candidate_score_delta: f32) -> CompetitiveDelta {
    ...
    let removed = if provider_degraded {
        Vec::new()
    } else {
    ...
    for k in old_keys.intersection(&new_keys) {
        ...
        if d.abs() >= threshold && (!provider_degraded || d > 0.0) {
            score_changes.push(CandidateScoreChange {
                candidate_key: k.clone(),
                name: if name_new.is_empty() {
                    name_old.clone()
                } else {
                    name_new.clone()
                },
    ...
    let new_asset_keys = new_asset_keys_set
        .difference(&old_asset_keys)
        .cloned()
        .collect::<Vec<_>>();
    ...
    let guidance_keys = old_guidance
        .keys()
        .chain(new_guidance.keys())
        .cloned()
        .collect::<BTreeSet<_>>();
    let mut changed_section_keys = guidance_keys
        .into_iter()
        .filter(|k| old_guidance.get(k) != new_guidance.get(k))
        .collect::<Vec<_>>();
    changed_section_keys.sort();
    changed_section_keys.dedup();

    let broad_components = [
        "market_summary",
    ...
    let broad_strategy_change = broad_components.iter().any(|name| {
        normalized_strategy_component(previous, name)
            != normalized_strategy_component(current, name)
    });
    let strategy_changed = previous.get("strategy_sha256") != current.get("strategy_sha256")
        || broad_strategy_change
        || !changed_section_keys.is_empty();
    ...
    let public_data_changed = !added.is_empty()
        || !removed.is_empty()
        || !score_changes.is_empty()
        || !new_asset_keys.is_empty()
        || !removed_asset_keys.is_empty();
    ...
    let summary = if old_run.is_none() {
        ...
        if !added.is_empty() {
            parts.push(format!(
                "{} new capability-matched organization(s)",
                added.len()
            ));
        }
        if !removed.is_empty() {
            parts.push(format!(
                "{} organization(s) no longer ranked",
                removed.len()
            ));
        }
        if !new_asset_keys.is_empty() {
            parts.push(format!(
                "{} new public competitive asset(s)",
                new_asset_keys.len()
            ));
        }
        if !removed_asset_keys.is_empty() {
            parts.push(format!(
                "{} previously observed asset(s) no longer returned",
                removed_asset_keys.len()
            ));
        }
        if !score_changes.is_empty() {
            parts.push(format!(
                "{} material competitor score change(s)",
                score_changes.len()
            ));
        }
        if strategy_changed && public_data_changed {
            parts.push("positioning strategy changed".into());
        }
        if provider_degraded {
            parts.push(
                "one or more public providers were degraded; missing/downward signals were ignored"
                    .into(),
            );
        }
        if parts.is_empty() {
            if strategy_changed {
                "No new public competitor evidence was detected; strategy wording changed but no grant text was auto-revised.".into()
            } else {
                "No material competitive change detected.".into()
            }
        } else {
            format!(
                "Competitive intelligence refresh detected {}.",
                parts.join(", ")
            )
        }
    };
