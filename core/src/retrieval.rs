...
impl RetrievalConfig {
    ...
    fn from_env() -> Self {
        ...
        Self {
            ...
            candidate_multiplier: std::env::var("RETRIEVAL_CANDIDATE_MULTIPLIER")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(4usize)
                .clamp(1, 32),
            openmp_threshold: std::env::var("RETRIEVAL_OPENMP_THRESHOLD")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(4096usize)
                .max(256),
            ...
            freshness_half_life_days: Self::env_f32("RETRIEVAL_FRESHNESS_HALF_LIFE_DAYS", 365.0)
                .max(1.0),
        }
    }
}
...
impl RetrievalEngine {
    pub async fn build(
        ...
        if records.is_empty() {
            anyhow::bail!("project has no retrieval records to index");
        }
        ...
        let result = async {
            let write_batch = std::env::var("INDEX_EMBED_BATCH_RECORDS")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(64usize)
                .clamp(1, 512);
            ...
            let dims = first
                .first()
                .context("embedding endpoint returned zero vectors")?
                .len();
            if first.iter().any(|v| v.len() != dims) {
                anyhow::bail!("inconsistent embedding dimensions");
            }
            let mut writer =
                MmapMatrixWriter::create(&staging.join("vectors.f32"), records.len(), dims)?;
            let first_flat: Vec<f32> = first.into_iter().flatten().collect();
            writer.write_rows(0, &first_flat)?;
            let mut start = first_end;
            while start < texts.len() {
                let end = (start + write_batch).min(texts.len());
                let batch = embed.embed_documents(&texts[start..end]).await?;
                if batch.len() != end - start || batch.iter().any(|v| v.len() != dims) {
                    anyhow::bail!("embedding batch shape mismatch at rows {start}..{end}");
                }
                let flat: Vec<f32> = batch.into_iter().flatten().collect();
                writer.write_rows(start, &flat)?;
                start = end;
            }
            writer.finish()?;
            ...
            let record_req: Vec<Option<String>> =
                records.iter().map(|r| r.requirement_id.clone()).collect();
            ...
            if final_fingerprint != input_fingerprint {
                anyhow::bail!(
                    "project knowledge changed during index build; discard staging index and retry"
                );
            }
        ...
        if project_dir.exists() {
            ...
            if let Err(e) = std::fs::rename(&staging, &project_dir) {
                let _ = std::fs::rename(&old, &project_dir);
                return Err(e.into());
            }
            let _ = std::fs::remove_dir_all(old);
        } else {
            std::fs::rename(&staging, &project_dir)?;
        }
        Ok(manifest)
    }
    ...
    pub fn retrieve(
        ...
        let lexical = self
            .lexical
            .scores(query_text, self.config.bm25_k1, self.config.bm25_b);
        let n = semantic.len();
        let now = time::OffsetDateTime::now_utc().unix_timestamp();
        let mut evidence = vec![0.0f32; n];
        let mut freshness = vec![0.0f32; n];
        for i in 0..n {
            let r = self.records.get(i)?;
            evidence[i] = match r.status.as_str() {
                "supported" => 1.0,
                ...
                "contradicted" => 0.05,
                _ => 0.5,
            } * r.confidence.clamp(0.0, 1.0).max(0.2);
            freshness[i] = match r.created_unix {
                Some(ts) => {
                    let age_days = ((now - ts).max(0) as f32) / 86_400.0;
                    2.0f32
                        .powf(-age_days / self.config.freshness_half_life_days)
                        .clamp(0.05, 1.0)
                }
                None => 0.5,
            };
        ...
        let pre = if n >= self.config.openmp_threshold {
            hpc::openmp_topk(&fused, candidate_k)
        } else {
            hpc::parallel_topk(&fused, candidate_k)
        };
        ...
        for (row, _) in pre.iter().take(k.max(1)) {
            ...
            if r.kind == "requirement" {
                if let Some(id) = r.requirement_id.as_deref() {
                    for linked in self.csr.rows_for(id) {
                        selected.insert(linked as usize);
                    }
                }
            }
        }
        ...
        for row in selected {
            ...
            let graph_boost = if pre_rows.contains(&row) {
                0.0
            } else {
...
impl RetrievalService {
    pub fn new(store: Arc<Store>, embed: Arc<EmbeddingClient>, workspace: PathBuf) -> Self {
        Self {
            store,
            embed,
            workspace,
    ...
    pub async fn ensure_index(&self, project: &str) -> Result<IndexManifest> {
        ...
        if let Ok(engine) = RetrievalEngine::open(&self.workspace, project) {
            if engine.manifest().fingerprint == fingerprint {
                return Ok(engine.manifest().clone());
            }
        }
        let lock = self.project_lock(project);
        let _guard = lock.lock().await;
    ...
    pub async fn search(&self, project: &str, query: &str, k: usize) -> Result<Vec<RetrievalHit>> {
        self.ensure_index(project).await?;
        let q = self.embed.embed_query(query).await?;
        let engine = RetrievalEngine::open(&self.workspace, project)?;
        engine.retrieve(&q, query, k)
    }
    pub fn status(&self, project: &str) -> Result<serde_json::Value> {
        let fingerprint = self.store.retrieval_fingerprint(project)?;
        match RetrievalEngine::open(&self.workspace, project) {
            Ok(e) => Ok(
                serde_json::json!({"ready":true,"fresh":e.manifest().fingerprint==fingerprint,"manifest":e.manifest(),"requirement_nodes":e.requirement_count(),"record_rows":e.record_count()}),
            ),
            Err(_) => {
                Ok(serde_json::json!({"ready":false,"fresh":false,"fingerprint":fingerprint}))
            }
        }
    }
}
