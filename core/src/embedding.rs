...
impl EmbeddingClient {
    pub fn from_env() -> Result<Self> {
        let timeout = std::env::var("EMBEDDING_HTTP_TIMEOUT_SECONDS")
            .ok().and_then(|v| v.parse().ok()).unwrap_or(120u64);
        let batch_size = std::env::var("EMBEDDING_BATCH_SIZE")
            .ok().and_then(|v| v.parse().ok()).unwrap_or(64usize).clamp(1, 512);
        Ok(Self {
            client: Client::builder().timeout(Duration::from_secs(timeout)).build()?,
            ...
            health_ttl: Duration::from_secs(std::env::var("EMBEDDING_HEALTH_TTL_SECONDS").ok().and_then(|v|v.parse().ok()).unwrap_or(600u64)),
    ...
    pub async fn health(&self)->Result<serde_json::Value>{
        if let Some((checked,dimensions))=*self.health_cache.lock() {
            if checked.elapsed() < self.health_ttl {
                return Ok(serde_json::json!({"ok":true,"model":self.model,"dimensions":dimensions,"cached":true}));
            }
        }
    ...
    pub async fn embed(&self, inputs: &[String]) -> Result<Vec<Vec<f32>>> {
        ...
        for chunk in inputs.chunks(self.batch_size) {
            ...
            if body.data.len() != chunk.len() {
                bail!("embedding endpoint returned {} vectors for {} inputs", body.data.len(), chunk.len());
            }
            for item in body.data {
                if item.embedding.is_empty() { bail!("embedding endpoint returned empty vector"); }
                match expected_dim {
                    None => expected_dim = Some(item.embedding.len()),
                    Some(d) if d != item.embedding.len() => bail!("embedding dimension changed within request: {d} -> {}", item.embedding.len()),
                    _ => {}
                }
    ...
    pub async fn embed_documents(&self, inputs: &[String]) -> Result<Vec<Vec<f32>>> {
        if self.document_prefix.is_empty() { return self.embed(inputs).await; }
        let prefixed: Vec<String> = inputs.iter().map(|x| format!("{}{}", self.document_prefix, x)).collect();
        self.embed(&prefixed).await
    }

    pub async fn embed_query(&self, input: &str) -> Result<Vec<f32>> {
        let text = if self.query_prefix.is_empty() { input.to_string() } else { format!("{}{}", self.query_prefix, input) };
